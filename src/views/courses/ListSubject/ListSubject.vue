<template>
 ...
 <feather type="chevron-right" size="8"></feather>
 <span class="text-blue-950">Subjects</span>
 </nav>
 <h2 class="text-2xl font-black text-blue-950 tracking-tight">Subjects</h2>
 </div>
 <button 
 ...
 <th v-for="head in ['S. No.', 'Categories', 'Subject', 'Sub Child', 'Type', 'Sequence', 'Status', 'Action']" :key="head" class="px-5 py-3 text-[10px] font-black text-blue-500 uppercase tracking-[0.2em]">
 <div class="flex items-center gap-1.5 grayscale opacity-70 hover:opacity-100 transition-all cursor-pointer group">
 {{ head }}
 <feather v-if="head !== 'Action' && head !== 'Status'" type="chevron-down" size="10" class="group-hover:translate-y-0.5 transition-transform"></feather>
 </div>
 </th>
 </tr>
 </thead>
 <tbody class="divide-y divide-blue-100/60">
 <tr v-for="(sub, index) in subjects" :key="sub.id" class="group hover:bg-blue-50/40 transition-all duration-300">
 <td class="px-5 py-3 text-[11px] font-normal text-black tabular-nums">{{ index + 1 }}</td>
 <td class="px-5 py-3 text-[11px] font-bold text-blue-900 uppercase tracking-tight">{{ sub.category }}</td>
 <td class="px-5 py-3 text-[11px] font-normal text-blue-950 font-bold">{{ sub.name }}</td>
 <td class="px-5 py-3 text-[11px] font-normal text-blue-500">{{ sub.subChild || '-' }}</td>
 <td class="px-5 py-3 text-[11px] font-normal text-blue-500">{{ sub.type }}</td>
 <td class="px-5 py-3 text-[11px] font-black text-black tabular-nums">{{ sub.sequence }}</td>
 <td class="px-5 py-3">
 <!-- Toggle Switch -->
 <div 
 @click="sub.status = !sub.status"
 :class="['w-10 h-5 rounded-full relative cursor-pointer transition-all duration-300 shadow-inner', sub.status ? 'bg-blue-600 shadow-blue-500/20' : 'bg-blue-200']"
 >
 <div :class="['absolute top-1 w-3 h-3 bg-white rounded-full transition-all duration-300 shadow-sm', sub.status ? 'left-6' : 'left-1']"></div>
 </div>
 </td>
 <td class="px-5 py-3">
 ...
 >
 <button @click.stop="handleAction('view', sub)" class="w-fit px-8 mx-auto sm:mx-0 text-left py-2 text-[11px] font-bold text-blue-600 hover:bg-blue-50 hover:text-blue-600 transition-all flex items-center gap-2">
 <feather type="eye" size="12"></feather> View
 </button>
 <button @click.stop="handleAction('edit', sub)" class="w-fit px-8 mx-auto sm:mx-0 text-left py-2 text-[11px] font-bold text-blue-600 hover:bg-blue-50 hover:text-blue-600 transition-all flex items-center gap-2">
 <feather type="edit-2" size="12"></feather> Edit
 </button>
 <div class="h-px bg-blue-100 my-1"></div>
 <button @click.stop="handleAction('delete', sub)" class="w-fit px-8 mx-auto sm:mx-0 text-left py-2 text-[11px] font-bold text-rose-600 hover:bg-rose-50 transition-all flex items-center gap-2">
 ...
 <span class="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Showing {{ subjects.length ? '1' : '0' }} to {{ subjects.length }} of {{ subjects.length }} entries</span>
...
export default {
 ...
 subjects: [
 { id: 1, category: 'Tech e-books (library)', name: 'Python', subChild: '', type: 'Parent', sequence: 0, status: true },
 { id: 2, category: 'Language', name: 'English Learning', subChild: '', type: 'Parent', sequence: 0, status: true }
 ]
 }
 },
 directives: {
