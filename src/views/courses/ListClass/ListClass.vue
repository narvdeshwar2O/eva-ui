<template>
 ...
 <feather type="chevron-right" size="8"></feather>
 <span class="text-blue-950">Class List</span>
 </nav>
 <h2 class="text-2xl font-black text-blue-950 tracking-tight">Classes</h2>
 </div>
 <button 
 ...
 <tr v-for="(cls, index) in classes" :key="cls.id" class="group hover:bg-blue-50/40 transition-all duration-300">
 <td class="px-5 py-3 text-[11px] font-normal text-black tabular-nums">{{ index + 1 }}</td>
 <td class="px-5 py-3 text-[11px] font-normal text-black tabular-nums">{{ cls.id }}</td>
 <td class="px-5 py-3 text-[11px] font-bold text-blue-900 tracking-tight">{{ cls.name }}</td>
 <td class="px-5 py-3">
 ...
 >
 <button @click.stop="handleAction('edit', cls)" class="w-fit px-8 mx-auto sm:mx-0 text-left py-2 text-[11px] font-bold text-blue-600 hover:bg-blue-50 hover:text-blue-600 transition-all flex items-center gap-2">
 <feather type="edit-2" size="12"></feather> Edit
 </button>
 <div class="h-px bg-blue-100 my-1"></div>
 <button @click.stop="handleAction('delete', cls)" class="w-fit px-8 mx-auto sm:mx-0 text-left py-2 text-[11px] font-bold text-rose-600 hover:bg-rose-50 transition-all flex items-center gap-2">
 ...
 <span class="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Showing 1 to {{ classes.length }} of {{ classes.length }} entries</span>
...
export default {
 ...
 methods: {
 handleAction(type, cls) {
 this.activeActionIndex = null;
 if (type === 'edit') {
 this.$emit('edit-class', cls);
 }
 console.log(`${type} class:`, cls.id);
 }
 }
}
</script>
