<template>
 ...
 <feather type="chevron-right" size="8"></feather>
 <span @click="$emit('back')" class="hover:text-blue-600 cursor-pointer transition-colors uppercase">FAQs</span>
 <feather type="chevron-right" size="8"></feather>
 <span class="text-blue-950 uppercase tracking-tight">{{ record ? 'Edit FAQ' : 'Create FAQ' }}</span>
 </nav>
 <h2 class="text-2xl font-black text-blue-950 tracking-tight">{{ record ? 'Edit FAQ' : 'Create FAQ' }}</h2>
 </div>
 <button 
 @click="$emit('back')"
 ...
 <!-- Question -->
 <div class="space-y-2">
 <label class="text-[11px] font-black text-blue-900 uppercase tracking-[0.2em]">Question <span class="text-rose-500">*</span></label>
 <input 
 ...
 <!-- Answer -->
 <div class="space-y-2">
 <label class="text-[11px] font-black text-blue-900 uppercase tracking-[0.2em]">Answer <span class="text-rose-500">*</span></label>
 <textarea 
 v-model="formData.answer"
 placeholder="Enter the detailed answer" 
 class="w-full h-40 p-5 rounded-2xl bg-blue-50 border border-blue-200 text-sm font-semibold focus:outline-none focus:border-blue-600 transition-all shadow-sm resize-none"
 required
 ></textarea>
 </div>
 ...
 {{ loading ? 'Processing...' : (record ? 'Update' : 'Save') }}
 </button>
 <button 
 type="button"
 @click="clearForm"
 ...
 <p class="text-[13px]">FAQ {{ record ? 'updated' : 'saved' }} successfully!</p>
 </div>
 </div>
 </div>
 </transition>
