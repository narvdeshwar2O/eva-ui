<template>
 ...
 <feather type="chevron-right" size="8"></feather>
 <span class="text-blue-950 uppercase tracking-tight">FAQs</span>
 </nav>
 <h2 class="text-2xl font-black text-blue-950 tracking-tight">FAQs</h2>
 </div>
 <button
 @click="$emit('add')"
 ...
 <th v-for="head in ['S.N', 'ID', 'Category', 'Question', 'Answer', 'Status', 'Actions']" :key="head" class="px-6 py-3 text-[10px] font-black text-blue-500 uppercase tracking-[0.2em]" :class="{'text-center': head === 'Actions'}">
 {{ head }}
 </th>
 </tr>
 </thead>
 ...
 <td colspan="8" class="px-6 py-10 text-center text-indigo-400 text-xs font-semibold">No data available in table</td>
 </tr>
 <tr v-for="(item, index) in filteredFaqs" :key="item.id" class="group hover:bg-blue-50/40 transition-all duration-300">
 <td class="px-6 py-3">
 <input type="checkbox" class="w-4 h-4 border-2 border-blue-200 rounded text-blue-600 focus:ring-blue-500 transition-all">
 </td>
 ...
 <td class="px-6 py-3 text-[11px] font-bold text-blue-600 uppercase tracking-wider">{{ item.category }}</td>
 <td class="px-6 py-3 text-[11px] font-bold text-blue-700">{{ item.question }}</td>
 <td class="px-6 py-3">
 <p class="text-[11px] text-blue-500 line-clamp-1 max-w-xs">{{ item.answer }}</p>
 </td>
 <td class="px-6 py-3">
 ...
 <span class="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Showing 1 to {{ faqs.length }} of {{ faqs.length }} entries</span>
...
export default {
 ...
 activeActions: null,
 faqs: [
 { id: 1, category: 'GENERAL', question: 'What is JSPARK.AI?', answer: 'JSPARK.AI is an advanced learning management platform for competitive exams.', status: true },
 { id: 2, category: 'COURSES', question: 'How can I enroll?', answer: 'You can enroll by selecting a course and clicking the "Buy Now" button.', status: true },
 { id: 3, category: 'PAYMENTS', question: 'Are refunds available?', answer: 'Refund policies vary by course. Please check the terms and conditions.', status: true }
 ]
 }
 },
 computed: {
